...
const state={token:hashToken,user:null,access:null,signals:{},history:[],payments:[],status:{},plans:{},clockOffsetMs:0,signalWindowDeadlineMs:0,nextBroadcastDeadlineMs:0,loading:false};
...
function renderAccess(){
 ...
 if(a?.paid&&a.plan){
   ...
   c.innerHTML=`<strong>${esc(p?.name||a.plan)} · ${money(planAmount(p),planCurrency())}</strong><span>Live signals are available. Plan active until ${new Date(a.expires).toLocaleString()}</span>`;
   return;
 }
 $("accessPlan").textContent="FREE";
 $("accessExpiry").textContent="Live access";
 c.innerHTML=`<strong>Free live access</strong><span>Live KETS signals are available to signed-in users without a paid plan.</span>`;
}
 ...
 grid.innerHTML=["BTC","GOLD"].map(m=>{
   ...
   if(noSetup){
     return `<article class="signal-card wait" data-signal-timestamp="${esc(s.timestamp||"")}">
       ...
       <div class="price">${money(s.current_price??s.price??s.market_price)}</div>
       ...
       <div class="metrics">
         ...
         <div class="metric"><span>Market price</span><strong>${money(s.current_price??s.price??s.market_price)}</strong></div>
       </div>
     </article>`;
   }
...
function renderHistory(){
 ...
 el.innerHTML=state.history.slice().reverse().slice(0,80).map(s=>{
  ...
  return `<div class="history-row">
    ...
    <div class="history-price">${money(price)}</div>
  </div>`;
 }).join("");
}
...
$("regCountry")?.addEventListener("change",updateCountryCurrency);updateCountryCurrency();$("loginBtn").onclick=login;$("registerBtn").onclick=register;$("profileBtn").onclick=openProfile;$("editProfileBtn").onclick=openProfile;
...
async function refreshDisplayedSignals(){
 ...
 try{
  ...
  const [signalFeed, historyFeed, statusFeed]=await Promise.all([
   api("/api/signals"),
   api("/api/history"),
   api("/api/status")
  ]);

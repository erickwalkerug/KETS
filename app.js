const API_BASE = (localStorage.getItem("KETS_API_BASE") || window.location.origin).replace(/\/$/, "");
...
function formatMove(v, pct) {
  ...
  return `${sign}${money(Math.abs(Number(v)))}${p}`;
}
  ...
  const cards = ["BTC","GOLD"].map(m => {
    ...
    return `<article class="signal-card ${dir}">
      ...
      <div class="metrics">
        ...
        <div class="metric"><span>Signal duration</span><strong>${activeDuration(s.timestamp)}</strong></div>
...
function startStatusTicker() {
  ...
  statusTick = setInterval(() => {
    const w = state.status.signal_window || {};
    if (w.active && Number(w.seconds_to_stop) > 0) w.seconds_to_stop--;
    if (!w.active && Number(w.seconds_to_start) > 0) w.seconds_to_start--;
    if (Number(state.status.next_broadcast_seconds) > 0) state.status.next_broadcast_seconds--;
    renderStatus();
    if (state.paid) renderSignals();
  }, 1000);
}
...
async function loadAll() {
  try {
    ...
    renderStatus(); renderSignals(); renderHistory(); startStatusTicker();
  } catch (e) {
    $("engineStatus").textContent = "Connection error";
    const grid = $("signalGrid");
    if (grid) grid.innerHTML = `<article class="signal-card"><div class="signal-top"><div class="market">KETS</div><div class="direction wait">OFFLINE</div></div><div class="price">--</div><div class="strength">Unable to load the dashboard data. Please refresh.</div></article>`;
    console.error("KETS API error:", e);
  }
}
...
// Register the service worker only when the file is actually available.
if ("serviceWorker" in navigator) {
  window.addEventListener("load", () => navigator.serviceWorker.register("/service-worker.js?v=20260827").catch(() => {}));
}

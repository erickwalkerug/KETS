...
function signalTimestamp(s){return s?.timestamp_utc||s?.timestamp||"";}
function parseSignalTime(s){const raw=signalTimestamp(s);const n=Date.parse(raw);return Number.isFinite(n)?n:NaN;}
function signalAge(s){const n=parseSignalTime(s);return Number.isFinite(n)?countdown((serverNowMs()-n)/1000):"--";}
...
function renderSignals(){
 ...
 grid.innerHTML=["BTC","GOLD"].map(m=>{
   ...
   if(!s)return `<article class="signal-card"><div class="signal-top"><div class="market">${m}</div><div class="direction wait">WAIT</div></div><div class="price">--</div><div class="strength">No current scan.</div></article>`;
   ...
   const price=s.current_price??s.price??s.market_price;
   const move=s.market_move??s.price_move??s.expected_price_move;
   const movePct=s.market_move_pct??s.price_move_pct??s.expected_price_move_percent;
   const expected=s.expected_move??s.expected_price_move??s.price_move;
   const expectedPct=s.expected_move_pct??s.expected_price_move_percent??s.price_move_pct;
   if(noSetup){
     const scanLabel=Number.isFinite(parseSignalTime(s))?new Date(parseSignalTime(s)).toLocaleString():(s.timestamp||"--");
     return `<article class="signal-card wait" data-signal-timestamp="${esc(ts)}">
       <div class="signal-top"><div class="market">KETS ${esc(s.market||s.asset||m)}</div><div class="direction wait">NOT QUALIFIED</div></div>
       <div class="price">${signalMoney(price)}</div>
       <div class="strength">No qualifying setup — monitoring the market.</div>
       <div class="metrics">
         <div class="metric"><span>Scan time</span><strong>${esc(scanLabel)}</strong></div>
         <div class="metric"><span>Scan age</span><strong>${signalAge(s)}</strong></div>
         <div class="metric"><span>Market price</span><strong>${signalMoney(price)}</strong></div>
         <div class="metric"><span>Market move</span><strong>${signalMoney(move)}${movePct!=null&&Number.isFinite(Number(movePct))?` (${Number(movePct).toFixed(2)}%)`:""}</strong></div>
       </div>
     </article>`;
   }
   const dir=String(s.direction||"WAIT").toUpperCase();
   const cls=dir==="BUY"||dir==="SELL"?dir.toLowerCase():"wait";
   const label=dir==="BUY"||dir==="SELL"?dir:"WAIT";
   const displayTs=Number.isFinite(parseSignalTime(s))?new Date(parseSignalTime(s)).toLocaleString():(s.timestamp||"--");
   return `<article class="signal-card ${cls}" data-signal-timestamp="${esc(ts)}">
     <div class="signal-top"><div class="market">KETS ${esc(s.market||s.asset||m)}</div><div class="direction ${cls}">${label}</div></div>
     <div class="price">${signalMoney(price)}</div>
     <div class="entry-line">Entry ${signalMoney(s.entry??s.price??s.market_price)}</div>
     <div class="strength"><span>${label} SIGNAL · Strength ${Number(s.score??s.strength??0)}%</span><div class="bar"><i style="width:${Math.min(100,Number(s.score??s.strength??0))}%"></i></div></div>
     <div class="metrics">
       <div class="metric"><span>Signal time</span><strong>${esc(displayTs)}</strong></div>
       <div class="metric"><span>Signal age</span><strong>${signalAge(s)}</strong></div>
       <div class="metric"><span>Market move</span><strong>${signalMoney(move)}${movePct!=null&&Number.isFinite(Number(movePct))?` (${Number(movePct).toFixed(2)}%)`:""}</strong></div>
       <div class="metric"><span>Expected move</span><strong>${signalMoney(expected)}${expectedPct!=null&&Number.isFinite(Number(expectedPct))?` (${Number(expectedPct).toFixed(2)}%)`:""}</strong></div>
       <div class="metric"><span>Estimated duration</span><strong>${esc(s.estimated_duration||s.duration_text||"--")}</strong></div>

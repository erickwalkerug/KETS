...
async function refreshDisplayedSignals(){
 ...
 try{
  ...
  state.signalMode=signalFeed.mode||"delayed";
  state.signalDelayMinutes=Number(signalFeed.delay_minutes||0);
  ...
  if(stamp){
   const mode=state.signalMode==="live"?"LIVE":"30M DELAY";
   stamp.textContent=`Updated ${new Date().toLocaleTimeString()} · ${mode}`;
  }
 }catch(e){

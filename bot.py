...
def api_google_start():
    ...
    if not client_id:
        return jsonify({"error": "Google Sign-In is not configured yet. Add GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET in Render."}), 503
    redirect_uri = KETS_PUBLIC_URL + "/api/auth/google/callback"
    state = _auth_serializer().dumps({"nonce": secrets.token_urlsafe(18)})
...
def api_google_callback():
    ...
    try:
        ...
        return "Invalid Google sign-in session.", 400
    if not code:
        return "Google sign-in was cancelled.", 400
    client_id = os.environ.get("GOOGLE_CLIENT_ID", "").strip()
    ...
    try:
        ...
        td = tr.json()
        ar = requests.get("https://openidconnect.googleapis.com/v1/userinfo",
                          headers={"Authorization": "Bearer " + td["access_token"]}, timeout=20)
        profile = ar.json()
        email = str(profile.get("email", "")).lower().strip()
        google_sub = str(profile.get("sub", "")).strip()
        if not email or not google_sub:
            raise RuntimeError("Google did not return a usable account.")
    except Exception as exc:
        return f"Google sign-in failed: {esc_html(str(exc))}", 502
    ...
    with DB_LOCK:
        ...
        if not row:
            row = conn.execute("SELECT * FROM users WHERE email=? COLLATE NOCASE", (email,)).fetchone()
        if row:
            conn.execute("UPDATE users SET google_sub=?, name=?, profile_picture=?, updated_at=? WHERE id=?",
                         (google_sub, profile.get("name",""), profile.get("picture",""), now, row["id"]))
            user_id = row["id"]
        else:
            user_id = str(uuid.uuid4())
            conn.execute("INSERT INTO users(id,email,password_hash,google_sub,name,profile_picture,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?)",
                         (user_id,email,None,google_sub,profile.get("name",""),profile.get("picture",""),now,now))
        conn.commit()
        row = conn.execute("SELECT * FROM users WHERE id=?", (user_id,)).fetchone()
        conn.close()
    token = _user_token(user_id)
    # Google callback returns a minimal bridge page; country onboarding is handled in the dashboard.
    token_json = json.dumps(token)
    return f"""<!doctype html><html><head><meta charset='utf-8'><title>KETS Sign In</title></head>
<body><p>Signing you in to KETS…</p><script>localStorage.setItem('kets_user_token',{token_json});window.location.replace('/?google=success');</script></body></html>"""
...
def api_profile():
    ...
    country_name = str(body.get("country_name", user["country_name"])).strip()[:100]
    country_code = str(body.get("country_code", user["country_code"])).strip().upper()[:8]
    profile_picture = str(body.get("profile_picture", user["profile_picture"])).strip()
    if len(profile_picture) > 1100000 or (profile_picture and not profile_picture.startswith(("data:image/","https://","http://"))):
        return jsonify({"error":"Profile picture is invalid or too large."}),400
    if COUNTRY_REQUIRED and (not country_name or not country_code):
        return jsonify({"error": "Country selection is required."}), 400
    with DB_LOCK:
        conn = db_conn()
        conn.execute("UPDATE users SET name=?,country_name=?,country_code=?,profile_picture=?,updated_at=? WHERE id=?",
...
def api_community():
    ...
    with DB_LOCK:
        conn = db_conn()
        rows = conn.execute("SELECT plan, COUNT(*) n FROM payments WHERE status='COMPLETED' AND updated_at IS NOT NULL GROUP BY plan").fetchall()
        conn.close()
    ...
    with DB_LOCK:
        conn = db_conn()
        rows = conn.execute("SELECT plan, user_id, created_at, updated_at FROM payments WHERE status='COMPLETED'").fetchall()
        conn.close()
    for row in rows:
...
def api_payment_history():
    ...
    with DB_LOCK:
        conn = db_conn()
        rows = conn.execute("SELECT tx_ref,plan,amount,currency,status,network,created_at,updated_at,tracking_id FROM payments WHERE user_id=? ORDER BY created_at DESC LIMIT 100",(user["id"],)).fetchall()
        conn.close()
...
def developer_data():
    ...
    with DB_LOCK:
        conn=db_conn()
        users=conn.execute("SELECT id,email,name,country_name,country_code,created_at FROM users ORDER BY created_at DESC LIMIT 500").fetchall()
        payments=conn.execute("SELECT * FROM payments ORDER BY created_at DESC LIMIT 500").fetchall()
        conn.close()
    return jsonify({"signals":signals,"markets":markets,"users":[dict(x) for x in users],"payments":[dict(x) for x in payments],"community":api_community().get_json() if False else None})
...
def _user_access(user_id):
    with DB_LOCK:
        conn=db_conn()
        rows=conn.execute("SELECT * FROM payments WHERE user_id=? AND status='COMPLETED' ORDER BY updated_at DESC",(user_id,)).fetchall()
        conn.close()
    active=[]
    for row in rows:
        exp=_subscription_expiry(row["plan"], row["updated_at"])
...
def _grant_payment_if_valid(tx_ref, tracking_id):
    ...
    with DB_LOCK:
        conn=db_conn()
        order=conn.execute("SELECT * FROM payments WHERE tx_ref=?",(tx_ref,)).fetchone()
        conn.close()
    if not order:
    ...
    with DB_LOCK:
        conn=db_conn()
        conn.execute("UPDATE payments SET tracking_id=?,status=?,amount=?,updated_at=? WHERE tx_ref=?",
...
def api_payment_create():
    ...
    with DB_LOCK:
        conn=db_conn()
        conn.execute("INSERT OR REPLACE INTO payments(id,user_id,tx_ref,tracking_id,plan,amount,currency,status,network,email,phone,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)",
                     (str(uuid.uuid4()),user["id"],tx_ref,tracking_id,plan_id,amount_to_pay,currency,"PENDING",network,email,phone,_now_iso(),_now_iso()))
        conn.commit()
        conn.close()
...
def api_payment_verify():
    ...
    with DB_LOCK:
        conn=db_conn()
        owner=conn.execute("SELECT user_id FROM payments WHERE tx_ref=?",(tx_ref,)).fetchone()
        conn.close()
    ...
    if not tracking_id:
        with DB_LOCK:
            conn=db_conn()
            row=conn.execute("SELECT tracking_id FROM payments WHERE tx_ref=?",(tx_ref,)).fetchone()
            conn.close()
...
def api_payment_callback():
    ...
    if result and result.get("paid"):
        with DB_LOCK:
            conn=db_conn()
            row=conn.execute("SELECT user_id FROM payments WHERE tx_ref=?",(tx_ref,)).fetchone()
            conn.close()

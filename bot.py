...
@app.get("/")
def root():
    # Serve the real KETS dashboard from the same Render service.
    return send_from_directory(FRONTEND_DIR, "index.html")

@app.get("/<path:filename>")
def frontend_files(filename):
    # Serve only known frontend assets. API routes are defined separately above.
    allowed = {"app.js", "styles.css", "manifest.json", "service-worker.js"}
...
# Start the background engine when imported by Gunicorn as well as when run directly.
# This is required because Gunicorn does not execute the __main__ block.

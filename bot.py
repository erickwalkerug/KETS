...
def _sync_signal_source_once():
    ...
    if not key:
        ...
        return False
    try:
        # History is the authoritative feed for both live and delayed access.
        r = requests.get(url + "/api/history", headers=_source_headers(key), timeout=15)
        print(f"📥 KETS source bridge: GET /api/history -> {r.status_code}")
        ...
        # Fetch the current feed too. Some source deployments expose current
        # signals separately from history. If it succeeds, use it as latest.
        r2 = requests.get(url + "/api/signals", headers=_source_headers(key), timeout=15)
        print(f"📥 KETS source bridge: GET /api/signals -> {r2.status_code}")
        current = {}
        if r2.status_code == 200:
            d2 = r2.json()
            if isinstance(d2, dict):
                current = d2.get("signals", {}) or {}
        ...
        if not latest:
            for item in history:
                ...
                old = latest.get(asset)
                if old is None or str(item.get("timestamp", "")) > str(old.get("timestamp", "")):
                    latest[asset] = item
        ...
        with API_LOCK:
            ...
            for x in history_snapshot:
                try:
                    ...
                    continue
                if dt >= now - datetime.timedelta(days=SIGNAL_HISTORY_DAYS):
                    clean.append(x)
